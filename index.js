...
module.exports = class Comci {
    static async getTimeTable(schoolId, grade, cl, nextweek = false) {
        ...
        var data = await axios.get("http://comci.net:4082" + url.split("?")[0] + "?" + base64);
    ...
    static async getSchoolNumber(schoolName) {
        ...
        var data = await axios.get(`http://comci.net:4082${url}${str}`);
        data = JSON.parse(data.data.replace(/\0/g, ""));
    ...
    static async #getHtml() {
        var result = await axios.get("http://comci.net:4082/st", {
